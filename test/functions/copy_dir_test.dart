  ...
  t.group('copyDir', () {
    ...
    t.test('empty target - overwrite', () {
      TestFileSystem().withinZone((fs) {
        ...
        createDir(to);
        copyDir(from, to);
        copyDir(from, to, overwrite: true);
    ...
    t.test('empty target - filter *.txt', () {
      TestFileSystem().withinZone((fs) {
        ...
        expected = subname(expected, 'top', 'new_top');
        createDir(to);
        copyDir(from, to, filter: (file) => extension(file) == '.txt');
    ...
    t.test('empty target - recursive ', () {
      TestFileSystem().withinZone((fs) {
        ...
        expected = subname(expected, 'top', 'new_top');
        createDir(to);
        copyDir(from, to, recursive: true);
    ...
    t.test('empty target - recursive- overwrite', () {
      TestFileSystem().withinZone((fs) {
        ...
        createDir(to);
        copyDir(from, to, recursive: true);
        copyDir(from, to, overwrite: true, recursive: true);
