...
The dcli-sdk package contains a collection of command line tools that are
desgined to assist with development of dart scripts.

Using the dcli-sdk is optional.

If you are looking for which package to import into your project then 
start with the 'dcli' package.

# Activation
To activate the dcli sdk tools run:

```bash
dart pub global activate dcli_sdk
```

You can then run the dcli command to see what tools are available:

```bash
dcli
```

Run the doctor command to help diagnose problems:
