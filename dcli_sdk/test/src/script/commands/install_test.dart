  ...
  group('Install DCli', () {
    test('warmup', () async {
      await withTestScope((testDir) async {
        expect(
          !core.Settings().isWindows || Shell.current.isPrivilegedUser,
          isTrue,
        );
        //TestFileSystem(useCommonPath: false).withinZone((fs) {

        try {
          await InstallCommand().run([], ['--noprivileges']);
...
void checkInstallStructure(String home) {
  ...
  expect(
    exists(truepath(home, '.dcli', 'template', 'project', 'custom')),
    isTrue,
  );
...
void checkScriptStructure(String home) {
  ...
  expect(
    exists(truepath(home, '.dcli', 'template', 'script', 'custom')),
    equals(true),
  );
  expect(
    exists(truepath(home, '.dcli', 'template', 'script', 'simple')),
    equals(true),
  );
  expect(
    exists(
      truepath(home, '.dcli', 'template', 'script', 'analysis_options.yaml'),
    ),
    equals(true),
  );
  expect(
    exists(truepath(home, '.dcli', 'template', 'script', 'pubspec.lock')),
    equals(true),
  );
  expect(
    exists(truepath(home, '.dcli', 'template', 'script', 'pubspec.yaml')),
    equals(true),
  );
}
