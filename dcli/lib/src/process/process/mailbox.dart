...
void check(int retval) {
  if (retval != 0) {
    throw MailboxException('FFI operaton failed');
  }
}
...
class Mailbox {
  ...
  static const int mutexSize = 64;
  static const int condSize = 64;
  ...
  Uint8List sendRequest(Uint8List message) {
    ...
    return _toList(lock(_mailbox.mutex, () {
      if (_mailbox.ref.state != stateNone) {
        throw MailboxException('Illegal Mailbox state');
      }
  ...
  void respond(Uint8List? message) {
    if (_mailbox.ref.state != stateNone) {
      throw MailboxException('Invalid state: ${_mailbox.ref.state}');
    }
...
class MailboxException extends DCliException {
  MailboxException(super.message);
}
